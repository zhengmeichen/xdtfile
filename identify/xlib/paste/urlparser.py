...
class URLParser(object):
    ...
    URLParser will also look in page.py for special overrides.
    These overrides are:
    ...
    def find_init_module(self, environ):
        filename = os.path.join(self.directory, 'page.py')
...
def load_module_from_name(environ, filename, module_name, errors):
    ...
    init_filename = os.path.join(os.path.dirname(filename), 'page.py')
    if not os.path.exists(init_filename):
        try:
            f = open(init_filename, 'w')
        except (OSError, IOError) as e:
            errors.write(
                'Cannot write page.py file into directory %s (%s)\n'
