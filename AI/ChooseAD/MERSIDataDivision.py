                    ...
                    if dims == 1:
                        ...
                        for i in range(data.shape[1]):
                            for j in range(data.shape[2]):
                                if group + '_' + k + '_' + str(i) + '_' + str(j) not in dict.keys():
                                    databool = data[:, i, j].flatten() != fillvalue
                                    if fillvalue not in data[:, i, j].flatten():
                                        if data[:, i, j].flatten() != []:
                                            dict[group + '_' + k + '_' + str(i) + '_' + str(j)] = \
                                            data[:, i, j].flatten()[databool]
                                else:
                                    databool = data[:, i, j].flatten() != fillvalue
                                    if fillvalue not in data[:, i, j].flatten():
                                        if data[:, i, j].flatten() != []:
                                            dict[group + '_' + k + '_' + str(i) + '_' + str(j)] = np.concatenate(
                                                [dict[group + '_' + k + '_' + str(i) + '_' + str(j)],
                                                 data[:, i, j].flatten()[databool]])
    return dict
...
if __name__ == '__main__':
    date = sys.argv[1]
    filelist = sorted(glob.glob(r'/FY3E/MERSI/1A/OBC/%s/%s/*.HDF' % (date[0:4], date)))
    # filelist = sorted(glob.glob('FY3E_MERSI_GRAN_1A_20220111_23*_OBC--_V0.HDF'))
    resultA = {}
    resultD = {}
    ...
    for key, value in dict.items():
        ...
        if len(Value_Count_key) < 10:
            ...
            resmin, resmax = CalAnalog(Alldict_Value)
            resultA[key] = [resmin, resmax]
    outputfileA = '/STSS/FY3E_STSS/dev/AIWarning/Param/MERSI/' + date + 'MERSI_A.pkl'
    outputfileD = '/STSS/FY3E_STSS/dev/AIWarning/Param/MERSI/' + date + 'MERSI_D.pkl'
    # outputfileA = '/STSS/FY3E_STSS/dev/AIWarning/Param/MERSI/MERSI_A.pkl'
    # outputfileD = '/STSS/FY3E_STSS/dev/AIWarning/Param/MERSI/MERSI_D.pkl'
