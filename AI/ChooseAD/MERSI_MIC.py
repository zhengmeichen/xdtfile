                            ...
                            if do_filter:
                                ...
                                if use_mean:
                                    if len(dict[group + '_' + k]) != len(data[:].flatten()[databool]):
                                        if min(len(dict[group + '_' + k]),len(data[:].flatten()[databool]))==0:
                                            continue
                                        len_limit = min(len(dict[group + '_' + k]),len(data[:].flatten()[databool]))
...
if __name__ == '__main__':
    file_re = '/FY3E/MERSI/1A/OBC/2022/20220701/FY3E_MERSI_GRAN_1A_*_OBC--_V0.HDF'
    out_path = '/STSS/FY3E_STSS/dev/AIWarning/MIC/MERSI/MERSI_MIC_sample.h5'
