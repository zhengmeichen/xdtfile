            ...
            for group in f.keys():
                for k in f[group].keys():
                    ...
                    if dims == 1:
                        ...
                        # 排序保证最长的数据在0，其次对其余维度进行循环
                        for i in range(data.shape[1]):
                            for j in range(data.shape[2]):
                                if group + '_' + k + '_' + str(i) + '_' + str(j) not in dict.keys():
                                    databool = data[:, i, j].flatten() != fillvalue
                                    # 只保留非填充值的维度数据  if fillvalue not in data[:, i, j].flatten():
                                    if fillvalue not in data[:, i, j].flatten():
                                        dict[group + '_' + k + '_' + str(i) + '_' + str(j)] = data[:, i, j].flatten()[databool]
...
if __name__ == '__main__':
    date = sys.argv[1]
    filelist = sorted(glob.glob(r'/FY3E/SIM/1A/%s/%s/*.HDF' % (date[0:4], date)))
    resultA = {}
    resultD = {}
    # part 0 将所有数据集均存放在字典中
    dict = ReadData(filelist[:10])
    ...
    for key, value in dict.items():
        ...
        if len(Value_Count_key) < 10:
            # Alldict_Valuebool = (
            #             (Alldict_Value == -9999.9) | (Alldict_Value == 65535) | (Alldict_Value == 4294934528) | (
            #             Alldict_Value == -32768) |
            ...
            resmin, resmax = CalAnalog(Alldict_Value)
            resultA[key] = [resmin, resmax]
    outputfileA = '/STSS/FY3E_STSS/dev/AIWarning/Param/SIM/' + date + 'SIM_A.pkl'
    outputfileD = '/STSS/FY3E_STSS/dev/AIWarning/Param/SIM/' + date + 'SIM_D.pkl'
