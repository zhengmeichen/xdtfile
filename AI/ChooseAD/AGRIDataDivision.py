                            ...
                            for i in range(data.shape[1]):
                                for j in range(data.shape[2]):
                                    if group + '_' + k + str(j) not in dict.keys():
                                        ...
                                        databool = data[:, i, j].flatten() != fillvalue
                                        dict[group + '_' + k + str(j)] = np.concatenate(
                                            [dict[group + '_' + k + str(j)], data[:, i, j].flatten()[databool]])
        except:
            continue
    return dict
...
def CalAnalog(value):
    ...
    if valuediffmedian==0:
        valuebool = valuediffabs / (valuemax - valuemin) <= 0.05
    else:
    ...
    uniquevalue = np.unique(sortedvalue)
    # for i in sortedvalue[1:10]:
    for i in uniquevalue[1:10]:#Anda
        # 判最小值
        if (i - finalmin) > 10 * diffmedian:
            finalmin = i
    # for i in sortedvalue[-10:-1]:
    for i in uniquevalue[-10:-1]:#Anda
        # 判最大值
...
if __name__ == '__main__':
    ...
    date = '202201'
    # date = sys.argv[1]
    # file_re = '/STSS/FY3E_STSS/dev/AIWarning/Data/FY4B/AGRI/'+date+'/FY4B_AGRI_'+date+'_*_DT.h5'
    file_re = '/STSS/FY3E_STSS/dev/AIWarning/Data/FY4B/AGRI/202201*/FY4B_AGRI_*_DT.h5'
    out_root = '/STSS/FY3E_STSS/dev/AIWarning/Param/AGRI/'
    # file_re = '/FY4BPDSPULL/OPER/FY4B/HDF/AGRI/202201*/FY4B-_AGRI--_*_00001.HDF'
    # out_root = '/FY4BMCSDATANAS/FY4B/OPER/AIWarning/Code/YAML/AGRI/'
            ...
            if len(Value_Count_key) < 10:
                ...
                if len(AllValue_Count.keys()) < 10:
                    if key not in resultD.keys():
                        AllValue_Count = count_filter(AllValue_Count, Alldict_Value,0.001)
    ...
    outputfileA = out_root + date + 'AGRI_A.pkl'
    outputfileD = out_root + date + 'AGRI_D.pkl'
