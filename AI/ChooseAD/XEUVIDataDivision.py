...
def ReadData(filelist):
    ...
    for eachfile in filelist:
        ...
        with h5py.File(eachfile, 'r') as f:
            for group in f.keys():
                if 'Calibration' != group:
                    for k in f[group].keys():
                        # group = group.encode('utf-8')
                        # k = k.encode('utf-8')
                        data = f[group + '/' + k][()]
                        # 获取每个数据集的填充值,XUEVI没有这个属性
                        ...
                        if dims == 1:
                            ...
                            for i in range(data.shape[1]):
                                if group + '_' + k + '_' + str(i) not in dict.keys():
                                    ...
                                    dict[group + '_' + k + '_' + str(i)] = data[:, i].flatten()
                                else:
                                    # databool = data[:, i].flatten() != fillvalue
                        ...
                        #             if group + '_' + k + '_' + str(j) not in dict.keys():
                        #                 # databool = data[:, i, j].flatten() != fillvalue
                        #                 dict[group + '_' + k + '_' + str(j)] = data[:, i, j].flatten()
                        #             else:
                        #                 # databool = data[:, i, j].flatten() != fillvalue
                        #                 dict[group + '_' + k + '_' + str(j)] = np.concatenate(
                        #                     [dict[group + '_' + k + '_' + str(j)], data[:, i, j].flatten()])
    return dict
...
if __name__ == '__main__':
    ...
    for key, value in dict.items():
        ...
        if len(Value_Count_key) < 10:
            ...
            resmin, resmax = CalAnalog(Alldict_Value)
            resultA[key] = [resmin, resmax]
    outputfileA = '/STSS/FY3E_STSS/dev/AIWarning/Param/XEUVI/' + date + 'XEUVI_A.pkl'
    outputfileD = '/STSS/FY3E_STSS/dev/AIWarning/Param/XEUVI/' + date + 'XEUVI_D.pkl'
