                ...
                if 'GROUP_TABLE' != group:
                    for k in f[group].keys():
                        if 'QA' not in k:
                            ...
                            # print group + '/' + k,'FillValue', fillvalue
...
if __name__ == '__main__':
    date = sys.argv[1]
    filelist = sorted(glob.glob(r'/FY3E/MWTS/1A/%s/%s/*.HDF' % (date[0:4], date)))
    resultA = {}
    resultD = {}
    # part 0 将所有数据集均存放在字典中
    dict = ReadData(filelist[:10])
    # print(dict['Calibration_NEdTCold0'])
    # exit()
    ...
    for key, value in dict.items():
        ...
        #         Alldict_Value == -32768) |
        #                      (Alldict_Value == 4294967295) | (Alldict_Value == -2147483648) |
        ...
        if len(Value_Count_key) < 10:
            ...
            resmin, resmax = CalAnalog(Alldict_Value)
            resultA[key] = [resmin, resmax]
    outputfileA = '/STSS/FY3E_STSS/dev/AIWarning/Param/MWTS/' + date + 'MWTS_A.pkl'
    outputfileD = '/STSS/FY3E_STSS/dev/AIWarning/Param/MWTS/' + date + 'MWTS_D.pkl'
