...
if __name__ == '__main__':
    date = sys.argv[1]
    # 只要连续的数据OBCSL
    filelist = sorted(glob.glob(r'/FY3E/SSIM/1A/OBCSL/%s/%s*/*.HDF' % (date[0:4], date)))
    resultA = {}
    resultD = {}
    # part 0 将所有数据集均存放在字典中
    # SSIM在2021/11~2022/2月之间有几天没有数据
    ...
    for key, value in dict.items():
        ...
        if len(Value_Count_key) < 10:
            ...
            resmin, resmax = CalAnalog(Alldict_Value)
            resultA[key] = [resmin, resmax]
    outputfileA = '/STSS/FY3E_STSS/dev/AIWarning/Param/SSIM/SSIMOBCSL/' + date + 'SSIMOBCSL_A.pkl'
    outputfileD = '/STSS/FY3E_STSS/dev/AIWarning/Param/SSIM/SSIMOBCSL/' + date + 'SSIMOBCSL_D.pkl'
