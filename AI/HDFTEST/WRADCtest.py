...
if __name__ == '__main__':
    date = sys.argv[1]
    # A
    filelistA = sorted(glob.glob(r'/FY3E/WRAD/1A/OBC/ASCENDC/%s/%s/*.HDF' % (date[0:4], date)))
    for i in filelistA:
        print(i)
        filename = i
        outfilename = '/STSS/FY3E_STSS/dev/AIWarning/HDFTEST/HDF/WRADC/ASCENDC/' + i.split('/')[-1]
    ...
    # D
    filelistD = sorted(glob.glob(r'/FY3E/WRAD/1A/OBC/DESCENDC/%s/%s/*.HDF' % (date[0:4], date)))
    for i in filelistD:
        print(i)
        filename = i
        outfilename = '/STSS/FY3E_STSS/dev/AIWarning/HDFTEST/HDF/WRADC/DESCENDC/' + i.split('/')[-1]
        with h5py.File(filename, 'r') as f, h5py.File(outfilename, 'w') as o:
            # PLAT时间
            DaycntA = (f['/Geolocation Fields/Daycnt'][:].astype('f4') * 24 * 60 * 60 * 1000).astype('i8')
            MscntA = (f['/Geolocation Fields/Mscnt'][:].astype('f4') / 10).astype('i8')
