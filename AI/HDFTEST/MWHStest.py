...
# 阈值范围
outputfileA = '/STSS/FY3E_STSS/dev/AIWarning/YAML/MWHS_A.pkl'
outputfileD = '/STSS/FY3E_STSS/dev/AIWarning/YAML/MWHS_D.pkl'
# outputfileA = 'C:\\Users\\19683\\Downloads\\MWHS_A.pkl'
# outputfileD = 'C:\\Users\\19683\\Downloads\\MWHS_D.pkl'
# 将AD字典读入
...
# filename = '/FY3E/MWHS/1A/2022/20220105/FY3E_MWHS-_ORBT_1A_20220105_2125_OBC--_V0.HDF'
# outfilename = '/STSS/FY3E_STSS/dev/AIWarning/Code/HDFTEST/HDF/FY3E_MWHS-_ORBT_1A_20220105_2125_OBC--_V0.HDF'


if __name__ == '__main__':
    date = sys.argv[1]
    filelist = sorted(glob.glob(r'/FY3E/MWHS/1A/%s/%s/*.HDF' % (date[0:4], date)))
    for i in filelist:
        print(i)
        filename = i
        outfilename = '/STSS/FY3E_STSS/dev/AIWarning/HDFTEST/HDF/MWHS/' + i.split('/')[-1]
        with h5py.File(filename, 'r') as f, h5py.File(outfilename, 'w') as o:
            # packgetime时间
            Daycnt = (f['/Geolocation/Scnlin_daycnt'][:].astype('f4') * 24 * 60 * 60 * 1000).astype('i8')
            Mscnt = (f['/Geolocation/Scnlin_mscnt'][:].astype('f4') * 0.1).astype('i8')

            fillvalueD = f['/Geolocation/Scnlin_daycnt'].attrs['FillValue']
            fillvalueM = f['/Geolocation/Scnlin_mscnt'].attrs['FillValue']
                        ...
                        for m in range(data.shape[1]):
                            for n in range(data.shape[2]):
                                ...
                                if dicname in dicD.keys():
                                    ...
                                    # 连续的不在范围内置为1，填充值置为2
                                    bool = ((data[:, m, n] < dicA[dicname][0] - 0.05 * dicA[dicname][1]) & (
                                                data[:, m, n] != fillvalue)) | \
                                           ((data[:, m, n] > dicA[dicname][1] + 0.05 * dicA[dicname][1]) & (
                                                       data[:, m, n] != fillvalue))
                                    fbool = (data[:, m, n] == fillvalue)
                                QAFlag[:, m, n][bool] = 1
                                QAFlag[:, m, n][fbool] = 2
